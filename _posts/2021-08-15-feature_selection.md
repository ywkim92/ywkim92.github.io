...
1. 기준(다중공선성, 낮은 t검정 통계량 등)에 따라 부적절한 features를 제거한 후 모델링하면 평가지표 상승을 기대할 수 있습니다. 모델을 단순화하여 과적합을 줄여줍니다.  
2. AI 모델링에 요구되는 정보의 수를 줄여 데이터 수집, 관리에 드는 비용을 절감할 수 있습니다. 예측이란 항상 불확실성을 동반하므로 초기 단계에서 데이터는 다다익선입니다. 그러나 다양한 소스에서 새로운 데이터가 계속 축적되고 있습니다. train & inference를 위해 많은 features가 필요하다면 이를 수집, 처리, 관리하는 것은 또 다른 부담입니다. 특정 feature의 양상, 경향이 바뀔 수도 있고 이상치 여부를 판단해야 하며 뜻하지 않은 이슈로 대량의 결측이 발생할 가능성도 염두에 두어야 합니다. 데이터 관리 및 모델링의 안정성 측면에서도 적절한 양의, 꼭 필요한, 최적화된 변수 집합 탐색을 반드시 고려해야 합니다.  
...
1. 필터: 카이 제곱 검정, 상관계수 등 통계학 이용
2. 래퍼: 가장 적합한 변수 부분 집합 탐색
3. 임베디드: LASSO, Ridge 등 모델 자체에 변수 선택을 위한 조건이 포함된 경우

이번 시간에는 래퍼 기법 중 recursive feature elimination, sequential feature selection과 임베디드 기법인 select from model을 이해하고 파이썬 코드로 구현하며 그 결과를 scikit-learn 라이브러리의 해당 함수와 비교해보겠습니다.
